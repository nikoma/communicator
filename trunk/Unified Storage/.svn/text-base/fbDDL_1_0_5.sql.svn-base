...
FOR SELECT FIRST (:iLIMIT)  MAX(CREATED) DATE_, CAST (CREATED AS DATE)  DAY_,  CAST (CREATED AS DATE) -  CAST ('NOW' AS DATE) DAYS_
FROM MESSAGES 
WHERE LOWER(CONTENT_TEXT) like LOWER(:iSEARCH) 
AND JID = LOWER(:iJID)
AND USER_ID =  :iUSER_ID 
GROUP by CAST (CREATED AS DATE) , CAST (CREATED AS DATE) -  CAST ('NOW' AS DATE) 
ORDER by DAY_ DESC
