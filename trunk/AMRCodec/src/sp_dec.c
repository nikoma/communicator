...
static int Post_Filter_init( Post_FilterState **state )
{
   ...
   *state = NULL;
   ...
   memset(s->agc_state, 0, sizeof(*s->agc_state));
...
static int Post_Process_init( Post_ProcessState **state )
{
   ...
   *state = NULL;
   ...
   /* allocate memory */
   s = (Post_ProcessState *) malloc(sizeof(Post_ProcessState));
   if (s == NULL )
      return -1;
...
void Speech_Decode_Frame_exit(void *st)
{
   ...
   s = (Speech_Decode_FrameState *)st;

   Decoder_amr_exit(&(s->decoder_amrState));
   Post_Filter_exit(&(s->post_state));
   Post_Process_exit(&(s->postHP_state));
...
void * Speech_Decode_Frame_init()
{
   ...
   /* allocate memory */
   s = (Speech_Decode_FrameState *)malloc(sizeof(Speech_Decode_FrameState));
   if (s == NULL )
	   return NULL;
